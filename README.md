...
Homebrew tap for installing the `canvas` CLI from `git54496/agent-canvas-pro`.

## Usage

```bash
brew tap git54496/agent-canvas-pro
brew install agent-canvas-pro
canvas doctor
```
...
3. Users run `brew update && brew upgrade agent-canvas-pro`.

Manual fallback if tap sync is unavailable:

```bash
./scripts/update_agent_canvas_formula.sh 0.13.1
git add Formula/agent-canvas-pro.rb dist/
git commit -m "Update agent-canvas-pro 0.13.1"
git push origin main
```

## Notes

- Formula file: `Formula/agent-canvas-pro.rb`
- Formula update helper: `scripts/update_agent_canvas_formula.sh`
- Source repository: `https://github.com/git54496/agent-canvas-pro`
- `canvas` comes from npm package `@agent-canvas/cli`; if npm global binaries come before Homebrew in PATH, run `which -a canvas` and remove `npm -g` installs as needed.
